...
const int mux_chnl_sel_2 = 26;      //Board pin number - Multiplexor channel selection S2
const int mux_chnl_sel_3 = 28;      //Board pin number - Multiplexor channel selection S3

const int trigger_1 = 36;           //Board pin number - Trigger of sensor #1
...
void loop() {
    ...
    for(j == 0; j < n_sensors; j++){        //Trigger the sensors and send data
        ...
        if(j == 0){
            ...
            capture_and_transmit();     //Capture echo signature and transmit via serial port         
        }
        else if(j == 1){
            ...
            //  Select MUX channel C2 (S0 = 0, S1 = 1, S2 = 0, S3 = 0)
            digitalWrite(mux_chnl_sel_0, LOW);
            digitalWrite(mux_chnl_sel_1, HIGH);
            digitalWrite(mux_chnl_sel_2, LOW);
            digitalWrite(mux_chnl_sel_3, LOW);

            //  Trigger the sensor 3 echo 
        ...
        delay(500); //wait 500ms until new measurement

    }
    delay(500); //wait 500ms until new measurement
}
/*************  Capture ADC data and send via Serial  *******************/
void capture_and_transmit(){
    ...
    for(i == 0; i < bufsize; i++){                  // Send data via serial port
        Serial.println(buf[obufn][i]*3.3/4095.0);   // Convert to voltage
